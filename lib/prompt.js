...
// adds despite being told to return only JSON. Uses proper brace-balance
// scanning (respecting quoted strings) to find exactly where the first
// JSON object starts and ends, ignoring any trailing text after it.
export function parseModelJson(rawText) {
  let cleaned = rawText
    .trim()
    .replace(/^```json/i, "")
  ...
  if (start === -1) {
    throw new Error(
      `No JSON object found in model response: ${cleaned.slice(0, 200)}`
    );
  }
  ...
  if (end === -1) {
    throw new Error(
      `Could not find a complete JSON object in model response: ${cleaned.slice(0, 200)}`
    );
  }
